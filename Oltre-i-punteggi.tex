...
\begin{verbatim}
Ultima modifica: 2025-09-26 09:39:24 CEST. Versione 0.1.0
\end{verbatim}
...
Un intero capitolo dedicato alla misurazione non può che partire
spiegando perchè è importante misurare. Accontentiamoci per il momento
di una definizione generica di misurazione come dell'utilizzo di numeri
per descrivere alcune proprietà (la definizione di misurazione è il tema
del \hyperref[misurazione_e_neuropsicologia_clinica]{prossimo
paragrafo}. Per farlo usiamo i \textbf{test} cioè specifici strumenti di
misurazione per la psicologia e neuropsicologia (i test sono oggetto di
un \hyperref[i-test]{paragrafo dedicato})

La misurazione è considerata una delle basi del metodo scientifico e un
approccio scientifico (e basato dall'evidenza), non può che passare
dalla misurazione degli oggetti di interesse. In breve quindi

\emph{misurare in neuropsicologia clinica ci permette di andare oltre la
semplice osservazione clinica}

Mentre in alcune branche della psicologia clinica la misurazione non è
sempre considerata necessaria, ma quando si parla di valutazione
neuropsicologica, la prima cosa che viene in mente è certamente una
persona che sta effettuando una serie di test per riuscire a comprendere
lo stato cognitivo di un individuo e una serie di punteggi ottenuti da
...
proprio una persona che somministra un \textbf{test}, verosimilmente in
qualità di strumento di misurazione (vedi paragrafo \hyperref[i-test]{I
test}.

Fermiamoci un attimo a riflettere su quale è lo scopo di una valutazione
neuropsicologica, come quella rappresentata nella figura sopra. Lo scopo
potrebbe essere l'accertamento sul sospetto di una patologia in corso
(es. un sospetto di demenza), o la caratterizzazione di un disturbo
cognitivo in una condizione accertata (es. capire se un trauma cranico
ha avuto delle conseguenze). Le informazioni possono avere fini
diagnostici, prognostici o riabilitativi. La neuropsicologia clinica
come disciplina \textbf{assume} che tramite misurazione degli oggetti di
interesse (funzionamento cognitivo, rischio di sviluppare una malattia,
etc), sia possibile raggiungere delle conclusioni diagnostiche migliori,
sia possibile fare delle migliori scelte terapeutiche e cliniche in
generale. La misurazione, cioè l'utilizzo di punteggi, ci aiuta a
quantificare delle proprietà di interesse e poter fare delle scelte
cliniche più precise rispetto al solo utilizzo della osservazione
clinica qualitativa. Il più delle volte questo implica che i punteggi
...
forse un rischio di avere una patologia neurodegenerativa.

Va da sé, che se questo è uno dei presupposti della neuropsicologica
clinica come disciplina, effettuare correttamente la misurazione è un
primo aspetto fondamentale che determinerà la qualità della nostra
attività clinica.

\section{La psicometria}\label{la-psicometria}
...
\hyperref[chaptersux2fAppendiciux2fdefinzioni.qmdux2fcostrutto]{\textbf{costrutti}},
cioè concetti psicologici non osservabili. Una caratteristica peculiare
della misurazione psicologica è che la relazione tra ciò che è l'esito
della misura (cioè il punteggio) e ciò che si vuole misurare, cioè il
costrutto è molto indiretto. Il fatto che ci sia un'intera disciplina
dedicato a questo sottolinea una cosa importante: la misurazione in
psicologia è un argomento complesso che non può essere banalizzato o
trascurato.

\begin{tcolorbox}[enhanced jigsaw, coltitle=black, colframe=quarto-callout-note-color-frame, arc=.35mm, bottomrule=.15mm, colback=white, rightrule=.15mm, titlerule=0mm, opacityback=0, bottomtitle=1mm, opacitybacktitle=0.6, title=\textcolor{quarto-callout-note-color}{\faInfo}\hspace{0.5em}{Definizione}, toptitle=1mm, left=2mm, leftrule=.75mm, breakable, toprule=.15mm, colbacktitle=quarto-callout-note-color!10!white]

\textbf{Costrutto} Un concetto psicologico non direttamente osservabile.
(La neuropsicologia la misurazione è spesso di costrutti non osservabili
come memoria, attenzione, etc.)

\end{tcolorbox}

La neuropsicologia tramite test e misurazione non si occupa però solo di
costrutti, ma anche di altri aspetti, più legati alla medicina in
generale. Ne è un esempio il rischio di sviluppare una patologia. Alcuni
test restituiscono come risultato proprio delle probabilità e in questo
caso l'oggetto di interesse non è dunque un costrutto.
...
proporietà del tavolo in una maniera per noi utile.

-- inserire qui una definizione migliore di misurazione --

Definiamo qui il \emph{Sistema Relazionale Empirico}, come l'insieme di
entità del mondo empirico che hanno una determinata proprietà che
vogliamo misurare (es. oggetti di diversa lunghezza) e il \emph{Sistema
Relazionale Numerico} un insieme di numeri (es. i numeri Reali) con i
quali vogliamo descrivere questa misura.
