...
Come per molti termini di questo libro, non esiste una definizione univoca di alidità. Di seguito ne è riportata una che cattura l'essenza di come è considerata la validità.
...
Un'altra possibile definizione di validità, con una sfumatura leggermente diversa è "la capacità di misurare quanto bene misura ciò che si propone di misurare, nel contesto in cui deve essere applicato"`(\cite{nunnally1994psychometric})`{=latex}`(Nunnally & Bernstein, 1994)`{=html}.
In questo caso è evidenziato un aspetto importante e cioè che la validità non è semplicemente una proprità del test, ma può cambiare a seconda del contesto in cui è utilizzato (questo aspetto sarà ripreso nel paragrafo "validità di una misurazione" (DA COMPLETARE).

È importante notare che _ciò che si intende misurare_ non è necessariamento un costrutto.
Un test potrebbe infatti avere come scopo quello di prevedere le possibilità di reinserimento a lavoro dopo il trauma cranico, oppure distinguere tra pazienti con Mild Cognitive Impairment (MCI) e soggetti con invecchiamento fisiologico. In tali casi l'obiettivo è chiaro e misurabile, ma non è, strettamente parlando, un concetto psicologico inosservabile.

Indipendentemte dall'esatta definizione la validità è associata alla presenza di evidenze scientifiche, di diverso tipo, che il test possa dare , dando una base per le interpretazioni ai punteggi.
Un test con buona validità per misurare la memoria avrà dunque evidenze scientifiche che effettivamente misuri la memoria. Un test che è in grado di distinguere tra pazienti con Mild Cognitive Impairment (MCI) e soggetti sani, ha evidenze scientifiche che effettivamente sia in grado di farlo efficacemente.
...
Questo può avvenire per diversi motivi: per esempio, potrebbero esserci strategie alternative per affrontare il test che non si basano sulla funzione cognitiva attesa, oppure le istruzioni potrebbero essere poco chiare e non essere affrontato nella maniera che dovrebbe coinvolgere la funzione cognitiva oggetto del test.
