...
## Introduzione alla misurazione {#introduzione-alla-misurazione}

:::{.domanda}
_Perché è importante misurare?_
:::

Prima di entrare nei dettagli è importante soffermarsi su questa domanda fondamentale. Non possiamo dare per scontato che misurare sia una cosa necessaria o indispensabile (in neuropsicologia clinica e in generale). Prima di tentare di rispondere a questo domanda dobbiamo chiarire cosa si intende per misurazione. Una definizione possibile, semplice, è la seguente:

_La misurazione è l'utilizzo di numeri per descrivere degli oggetti di interesse_.

Come vedremo nel [prossimo paragrafo](#misurazione_e_neuropsicologia_clinica), esistono svariate possibili definizioni di misurazione, con implicazioni non banali, ma accontentiamoci per il momento di questa per fare alcune prime considerazioni. Innanzitutto, misurare quasi sempre è associato ad ottenere _numeri_, cioè punteggi, che ci aiutano nella descrizione di qualcosa. Questi _oggetti di interesse_ in neuropsicologia sono di diverso tipo, ma il più delle volte sono funzioni cognitive, come attenzione o memoria. <!-- capire se vuoi generalizzare rispetto ad oggetti -->
...
<!-- Per Parte II – Approfondimenti tecnici
...
[^supp]: questo aspetto è affrontato più in dettaglio nella _Parte II – Approfondimenti tecnici_
...
Questi esempi, per quanto informali, mostrano come interpretare i punteggi grezzi di un test come se fossero su scala a intervalli abbia senz'altro delle criticità. Eppure, la maggior parte dei test neuropsicologici non riporta evidenze o non effettua verifiche che mostrino l'effettiva adeguatezza dell'utilizzo di una scala ad intervalli.
...
Ricapitolando: spesso in neuropsicologia si fa un utilizzo dei punteggi che, implicitamente, è affine alla concezione della misurazione proposta da Stevens e nello specifico assume che i punteggi possano essere trattati misure su scala a intervalli, senza però esplicitare o verificare l'adeguatezza di tale assunzione. 
Gli esempi forniti sopra sul MoCA e del Digit Span mostrano come questo presupposto non sia privo di problematiche.
Va tuttavia chiarito che la/il neuropsicologa/o non deve porsi quotidianamente il problema della scala di misurazione, a meno che non decida di effettuare operazioni sui punteggi al di là di quelle previste dalla pratica clinica standard. 
...
Un altro modo per affrontare la questione della misurazione in neuropsicologia clinica è focalizzarsi non tanto sul processo di misurazione, ma sugli strumenti che usiamo per fare la misurazione e su quale è il significato dei punteggi che tali strumenti producono. Questo è il presupposto della Teoria Classica dei Test, il framework più utilizzato nello sviluppo dei test psicologici e neuropsicologici[^ctt-meas] (si veda @teoria-classica-dei-test).

Prima di passare alla teoria classica dei test, cambiamo quindi prospettiva e dedichiamo adesso dello spazio a quelli che sono gli strumenti di misurazione in neuropsicologia: i test.


[^ctt-meas]: ad essere precisi, si noti che anche la teoria classica dei test che può essere a sua volta pensata come un modello di misurazione, sebbene non privo di problematiche `(\cite{borsboom2005measuring})`{=latex}`Borsboom (2005)`{=html}. 
...
Il capitolo si è finora focalizzato sulla Teoria Classica dei Test, sulla base delle attuali prassi di sviluppo dei test neuropsicologici (in Italia e non solo). Esistono però prospettive alla misurazione e allo sviluppo di test, spesso in grado di risolvere quei problemi che abbiamo sottolineato (es. l'assunzione non dimostrata di misurazione su scala ad intervalli, o i problemi della CTT).

### Introduzione all'Item Response Theory
Il principale approccio alternativo alla Teoria Classica dei Test è quella famiglia di modelli noti come Teoria di Risposta all'Item, o _Item Response Theory_ (IRT) (si veda anche _Parte II – Approfondimenti tecnici_, Item Response Theory).
L'Item Response Theory nasce con presupposti diversi rispetto alla Classical Test Theory così come in contrapposizione alla permissività della definizione di misurazione di Stevens. Da un punto di vista psicometrico è spesso considerata un modello fondato su basi più solide e privo di molte delle contraddizioni o limiti della CTT. Ad esempio, Denny Borsboom nel suo libro "Measuring the Mind" sostiene esplicitamente che la IRT rappresenta un "sostanziale miglioramento" rispetto alla CTT `(\cite{borsboom2005measuring}, p.81)`{=latex}`(Borsboom, 2005, p.81)`{=html}.
...
## Focus: i test sono veramente oggettivi?{#focus-test-oggettivi}
Uno dei motivi per cui i test sono utilizzati nella valutazione neuropsicologica è quello di permettere una valutazione che sia più _oggettiva_ rispetto ad una valutazione basata esclusivamente sul colloquio clinico o su aspetti qualitativi. 
Ricordiamo qui, che per _oggettivo_ si intende, indipendente dall'esaminatore.
Per rispondere alla domanda che fa da titolo a questo paragrafo, probabilmente è una domanda generica mal posta. I test non sono mai completamente oggettivi e ci sono diversi momenti e caratteristiche in cui l'oggettività può venire meno. Il modo in cui sono spiegate le istruzioni, potrebbe influenzare la prestazione, la sola presenza della/del neuropsicologa/o, potrebbe influenzare la prestazione, se è un test che richiede partecipazione attiva del clinico (es. elecando degli elementi da ricordare), può essere influenzata dalla dal tono della voce, dal volume o dalla cadenza. Anche l'attribuzione dei punteggi può essere più o meno dipendente dall'operatore. 
È probabilmente più corretto distinguere in test più o meno oggettivi e questo dipende dalle intrinseche caratteristiche del test. Quella dell'oggettività però non è solo un aspetto che si valuta qualitativamente sulla base delle caratteristiche del test, ma si può anche quantificare tramite indici, il più comune dei quali è l' _Inter-rater agreement_ , descritto nel par. {#inter-rater-agreement}: più alto è questo indice, più alta l'oggettività del test. L'oggettività della valutazione è anche discussa nel capitolo 11.
...
## Errori comuni: dedurre ciò che un test misura dal suo nome {#err-nome-test}

Un errore frequente nella valutazione di un test consiste nel dedurre ciò che misura unicamente dal suo nome. Il nome può certamente fornire una prima indicazione sul suo obiettivo, ma rimane un’etichetta: riflette il costrutto cheil test _dovrebbe_ misurare nelle intenzioni di chi lo ha sviluppato. Non costituisce, di per sé, una prova che i punteggi ottenuti possano essere interpretati come indicatori validi di quel costrutto.

Per stabilire se tale interpretazione sia giustificata, occorre esaminare come il test è stato costruito, quali processi sono coinvolti nello svolgimento dei compiti e, soprattutto, quali evidenze empiriche ne sostengono l’impiego nella popolazione e nel contesto di interesse. Il prossimo capitolo, dedicato a [validità e affidabilità](Validita-Affidabilita.qmd#validita-e-affidabilita), presenta i criteri con cui valutare queste evidenze.

`\pagebreak`{=latex}

:::{.callout-important}
Il nome di un test non ci dice _necessariamente_ cosa misura quel test. Ad esempio, un test che si presenta come "test di memoria di lavoro" non è detto misuri efficacemente la memoria di lavoro. Dovremmo sempre indagare se ci sono prove empiriche sufficienti che il test misuri con efficacia ciò che intende misurare.
:::
