...
Un esempio particolarmente illuminante è il termometro a mercurio. La _temperatura_ di un corpo non è direttamente osservabile: non si può "toccare" la temperatura in sé, ma solo percepire i suoi effetti. Tuttavia, possiamo misurarla grazie a un indicatore fisico osservabile: l'espansione di una colonna di mercurio all'interno di un tubo calibrato. Quando la temperatura aumenta, il mercurio si espande e sale, fornendo un numero (es. in gradi Celsius) che rappresenta indirettamente la proprietà fisica che ci interessa. Nessuno mette in dubbio la validità di questa misurazione perché sappiamo con precisione la relazione fisica tra la temperatura e l'espansione del mercurio.

La misurazione psicologica funziona in modo analogo, ma con una complicazione importante: nei costrutti psicologici la relazione tra il costrutto non osservabile (es. la memoria) e l'indicatore osservabile (es. le risposte a un test) non è mai altrettanto diretta o univocamente definita. Ecco perché la psicometria, come disciplina, si occupa proprio di come rendere questa relazione il più precisa e giustificata possibile.
...
_NOTA: forse aggiungere definizione breve di Rasch come ponte per IRT_
...
Ricapitolando questo paragrafo, la maggior parte dei test neuropsicologici adotta implicitamente un modello di misura a scala ad intervalli senza dichiararlo esplicitamente. Questa assunzione è problematica, come mostrano gli esempi del MoCA e del Digit Span, e lo è ancor di più quando si applicano correzioni demografiche per età e scolarità. Sebbene esistano test sviluppati su modelli di misurazione più rigorosi (come l'Item Response Theory), questi sono purtroppo ancora poco diffusi nella pratica clinica italiana.
...
A prima vista sembrerebbe di sì, in fondo stiamo chiedendo di ricordare delle parole. Eppure ci sono molte domande a cui questo test ipotetico non dà risposta: Perché 7 parole e non 5 o 10? Il ritardo di 30 secondi è sufficiente a distinguere una memoria normale da una deficitaria? Le 7 parole specifiche influenzano il punteggio (una parola ad alta frequenza come _casa_ è più facile da ricordare di una a bassa frequenza)? Il test misura la memoria a breve termine, quella di lavoro, o quella episodica? Cosa succede se il paziente è distratto, stanco o ha un deficit di attenzione: il punteggio riflette davvero la sua memoria? 
Si potrebbe rispondere che 7 parole sono poche per un buon test, ma quale è allora un numero sufficiente? Il problema è solo rimandato e ci servirebbe un riferimento per capire quando il numero è sufficiente.

Questo esempio illustra un principio fondamentale: soddisfare formalmente le definizioni di test e di misurazione che abbiamo fornito finora sono condizioni _necessarie_ ma non _sufficiente_ per avere uno strumento clinicamente utile. Il fatto che abbiamo assegnato dei numeri non garantisce che quei numeri misurino ciò che ci interessa, né che lo misurino in modo accurato e affidabile. La **validità** (la giustificazione del legame tra punteggio e costrutto) e l'**affidabilità** (la precisione e la consistenza della misura) sono gli aspetti più critici della psicometria e saranno trattati in dettaglio nei capitoli dedicati.
