...
In questo caso possiamo dire che il nostro Sistema Relazionale Empirico[^sis] è dato dall'insieme di tutte le persone con diversa memoria e il Sistema Relazionale Numerico come l'insieme di numeri che utilizziamo per descrivere la memoria di queste persone. 
...
Applicare lo stesso ragionamento a un test neuropsicologico è meno scontato. Prendiamo il MoCA: se fosse davvero una scala a intervalli, un aumento di 2 punti dovrebbe avere lo stesso significato sia partendo da 20 sia partendo da 28. Eppure un aumento da 28 a 30 porta a un punteggio perfetto, mentre uno da 26 a 28 indica semplicemente una prestazione medio-alta che migliora[^mis3]: è quantomeno discutibile sostenere che le due variazioni abbiano lo stesso significato.

Lo stesso vale per il Digit Span, test in cui si chiede di ricordare sequenze di numeri di lunghezza crescente e il punteggio corrisponde alla sequenza più lunga correttamente richiamata. Un aumento di 2 punti da 3 a 5 segna il passaggio da una prestazione molto bassa a una nella norma; un aumento da 8 a 10 porta invece da una prestazione già eccellente a una quasi irraggiungibile (provate a ricordare una sequenza di 10 cifre per capire). Anche qui le due variazioni potrebbero non sembrano equivalenti.

Questi esempi mostrano come interpretare i punteggi grezzi di un test come se fossero su scala a intervalli non si possa dare per scontato.
...
In questo paragrafo abbiamo visto come molte delle operazioni che si effettuano sui punteggi di test neuropsicologici assumono che le misure siano su una scala ad intervalli. Nel corso del prossimo paragrafo, tratteremo invece un altro aspetto spesso implicito nei test neuropsicologici e cioè il framework in cui sono sviluppati e in cui si verifica la loro bontà. Questo framework è noto come **Teoria Classica dei Test** (CTT, _Classical Test Theory_).
