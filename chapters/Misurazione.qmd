...
|                                                 | **Strumento preciso** | **Strumento impreciso** |
|:-------------------------------------------------:|:----------------------------------------------:|:-----------------------------------:|
...
:Le quattro tipologie di scale di misurazione secondo S.S. Stevens.
...
Tutte queste considerazioni e queste scale potrebbero sembrare una complicazione eccessiva, un sofismo, e comunque di scarso impatto a livello di pratica clinica. Ma ciò non è vero. Facciamo un breve esempio. In neuropsicologia clinica è molto diffusa la pratica di ottenere punteggi "corretti" cioè punteggi a cui è tolto il contributo di alcune variabili, spesso età e scolarità, ma non solo.
L'ottenere una correzione per età e scolarità presuppone che si possa sottrarre il contributo di queste variabili al punteggio in modo che i valori di individui con età e scolarità diverse siano comparibili (questo argomento sarà approfondito nel [Capitolo 2](#chapters/Identificare-deficit)). Ad esempio, se il mio punteggio originale è 8, ma il contributo stimato di età, scolarità e sesso di un individuo è 3, per rimuoverlo devo sottrarlo e il punteggio corretto sarà $8-3=5$. Questo operazione (di correzione) ha senso solo se la misurazione è a livello di scala ad intervalli o scala a rapporti. Se non lo fosse, la mia operazione sarebbe sbagliata e potrebbe portarmi ad un errore di utilizzo del punteggio e della sua interpretazione.
Di fatto, nella maggior parte dei test neuropsicologici _si assume_ che la misura sia su una Scala a intervalli o a rapporti ma, come sarà ripreso nel paragrafo [_Introduzione all'Item Response Theory_](#introduzione-item-response-theory)) e nel capitolo [_Misurare Cambiamenti nel  Tempo_](chapters/Cambiamenti-nel-tempo.qmd#misurare_cambiamenti), questa è un'assunzione che potrebbe non essere rispettata, rendendo l'utilizzo di certe operazioni improprio.
Vorrei chiarire che il neuropsicologo (clinico o forense), non dovrebbe porsi quotidianamente il problema sulla scala di misurazione del test che sta usando, a meno che non decida di fare delle operazioni sui punteggi che non sono quelle normalmente previste per la pratica clinica e dal test stesso. Il messaggio importante è: **il fatto che abbiamo ottenuto dei numeri non ci permette di fare ciò che vogliamo di questi numeri**. Alcune operazioni anche se praticamente possibili potrebbero non avere alcun senso.

Un altro aspetto importante da ricordare è che il fatto che abbia senso fare certe operazioni su un certo punteggio non ci garantisce che a livello di costrutto l'operazione abbia lo stesso senso. Prendiamo per esempio il test TMT-A. È un test molto utilizzato in cui viene presentato in un foglio una serie di numeri distribuiti in maniera sparsa. A chi deve svolgere il test è chiesto di unire nel minore tempo possibile i numeri in ordine crescente (quindi 1, poi 2, poi 3, etc.). Ne esistono versioni anche digitali, in cui i numeri devono essere uniti, o semplicemente toccati nell'ordine corretto. Il costrutto misurato dal TMT-A non è così univoco, ma per brevità qui assumiamo che misuri l'attenzione visuospaziale. Nel TMT-A il punteggio finale è spesso espresso in termini di tempo necessario per il completamento, in secondi. Il tempo è misurato secondo una scala a rapporti e questo risulta ovvio se proviamo ad applicare certe operazioni. Per esempio, chi impiega 80 secondi, ha impiegato il doppio del tempo di chi ne ha impiegati 40. Lo zero ha un significato preciso. Ma fermiamoci a riflettere: si può dire che la persona che ha completato in 80 secondi il TMT-A ha il _doppio_ dell'attenzione visuospaziale della persona che lo ha completato 40 secondi? Verosimilmente no e questo fa sì che la misura non sia in una scala a rapporti.

La disciplina che si occupa della misurazione in psicologia è la _psicometria_, una delle materie insieme alla statistica, che rappresentano le fondamenta di questo libro. La psicometria si occupa proprio delle questioni della misurazione in psicologia, una tipologia di misurazione molto particolare perchè non tratta di proprietà di oggetti fisici ma di concetti psicologici. Una caratteristica peculiare della misurazione psicologica è che la relazione tra ciò che è l'esito della misura (cioè il punteggio) e ciò che si vuole misurare, cioè il costrutto è  molto indiretto. Il fatto che ci sia un'intera disciplina dedicato a questo sottolinea una cosa importante: la misurazione in psicologia è un argomento complesso che non può essere banalizzato o trascurato.
