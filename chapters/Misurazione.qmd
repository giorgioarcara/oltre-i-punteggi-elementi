...
## Errori comuni: il nome del test non ci dice (necessariamente) cosa misura {#err-nome-test}
Un errore che ho visto di frequente nella valutazione di un test è quello di credere ciecamente nel nome del test per stabilire cosa effettivamente misura. Il nome del test è semplicemente un etichetta che descrive cosa il test _dovrebbe_ misurare secondo le intenzioni e i propositi di chi ha sviluppato il test. La plausibilità di un test o la sua apparenza, e tantomeno il nome, non sono però sufficienti a dirci cosa realmente il test misura. Il prossimo capitolo, dedicato a [validità ed affidabilità](#validita-e-affidabilita), tratta proprio questi aspetti e come un test dovrebbe mostrare evidenza di misurare ciò che intende misurare.
