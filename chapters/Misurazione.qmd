...
Validità e affidabilità sono le due proprietà fondamentali a cui fare riferimento per stabilire se un test è un buon test, e si collegano direttamente ai principi della Classical Test Theory (e in particolare la formula $X = T + E$, vedi il parafrago [Teoria Classica dei test](#teoria-classica-dei-test)) La validità risponde alla domanda _cosa stiamo misurando?_, cioè se il punteggio vero $T$ rifletta effettivamente il costrutto che il test intende misurare. L'affidabilità risponde invece alla domanda _quanto precisamente lo misuriamo?_, cioè quanto è piccola la componente d'errore $E$ rispetto al punteggio osservato $X$. Nei prossimi paragrafi tratteremo prima la validità e poi, in una sezione dedicata, l'affidabilità.
...
In questo caso è messo in evidenza un aspetto importante e cioè che la validità non è semplicemente una proprità del test, ma può cambiare a seconda del contesto in cui è utilizzato (questo aspetto sarà ripreso nel paragrafo "validità di una specifica misurazione" (DA COMPLETARE).

È importante notare che _ciò che si intende misurare_ non è necessariamento un costrutto (come la memoria, o l'attenzione).
Un test potrebbe infatti avere uno scopo diverso dalla misurazione di un concetto psicologico inosservabile: sono esempi pratici quello di prevedere le possibilità di reinserimento a lavoro dopo il trauma cranico, oppure distinguere tra pazienti con Mild Cognitive Impairment (MCI) e soggetti con invecchiamento fisiologico. In tali casi l'obiettivo è chiaro e misurabile ma non è, strettamente parlando, un «costrutto».

Indipendentemte dall'esatta di cosa sia la validità, essa è associata alla presenza di evidenze scientifiche, di diverso tipo, che il test possa dare , dando una base per le interpretazioni ai punteggi.
