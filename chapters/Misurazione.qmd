...
Vale la pena fermarci un secondo a riflettere su questo aspetto perché non esiste un riferimento unanimamente accettato su quali sono gli aspetti che andrebbero valutati in una valutazione neuropsicologica. I riferimenti più importanti sono le Linee Guida nazionali e internazionali specifiche per la patologia di interesse.
Queste fanno in genere riferimento a _domini_ che dovrebbero essere indagati nella valutazione di una specifica patologia (es. Linee Guida per Sclerosi Multipla, Linee Guida per Stroke, Linee Guida per Disturbo Cognitivo Legato al Cancro) e possono anche fare riferimento a specifici test che andrebbero utilizzati.
Alcuni manuali classici sulla valutazione neuropsicologica, sia disponibili in Italiano, sia in inglese (es. `\cite{lezak2012neuropsychological}`{=latex}`Lezak et al., 2012`{=html}, o il Manuale di Neuropsicologia `(\cite{vallar2018manuale})`{=latex}`(Vallar & Papagno, 2018)`{=html}), possono avere specifici capitoli dedicati a patologie o domini da valutare (anche in questo caso con possibili suggerimenti sugli strumenti da usare).
Queste indicazioni sono però sempre orientative e i test effettivamente disponibili sono sempre molto di più di quelli che sono citati in un manuale generico sull'attività clinica. Questo è anche perché un manuale è inevitabilmente una fotografia di un momento nel tempo e può non essere aggiornato alle più recenti linee guida o sviluppi.
Gli aspetti che sono rilevanti da valutare dipendono in generale dallo scopo della valutazione, dal contesto e dalle necessità cliniche e si rimanda soprattutto alle linee guida aggiornate o a manuali ed articoli specifici per patologia per conoscere quali domini o aspetti del funzionamento cognitivo andrebbero indagati.

Nel contesto di questa grande eterogeneità, da un punto di vista più metodologico è però utile fare delle considerazioni generali su come possono essere raggruppati gli oggetti di interesse degli strumenti neuropsicologici `(\cite{mondini2022methodology})`{=latex}`(Mondini, Cappelletti, & Arcara, 2022)`{=html}.

1. _funzioni cognitive o costrutti specifici_: il primo, e più classico, oggetto di interesse per un test neuropsicologico è un costrutto psicologico e nello specifico una funzione cognitiva, come potrebbe essere la memoria a breve termine, o l'attenzione selettiva. Questi test fanno parte spesso della valutazione chiamata di _secondo livello_, cioè quella che approfondisce una valutazione iniziale, più ampia ma approssimativa, fatta con test detti di _screening_. La valutazione neuropsicologica classica è però considerata come un insieme di test approfonditi che vanno ad esaminare vari aspetti del funzionamento cognitivo, proprio tramite una serie di test volti ad indagare funzioni cognitive specifiche. Anche se ogni test non misura mai esattamente un costrutto specifico `(\cite{sivan1999cognitive,lezak2012neuropsychological})`{=latex}`(Sivan & Benton, 1999; Lezak et al., 2012)`{=html}, è utile considerare questi test come il più alto livello di granularità che può essere raggiunto.
Si noti come gran parte dei test che fanno parte di questo gruppo sono utilizzati perché associati ad un'utilità clinica o perché sono importanti storicamente (es. il Token Test; `\cite{derenzi1962token}`{=latex}`De Renzi & Vignolo, 1962`{=html}). Ad un'analisi più approfondita non è però chiaro quale debba essere il livello di dettaglio che un test debba raggiungere quando è usato, in una valutazione neuropsicologica, per indagare l'integrità di una funzione cognitiva. Se infatti prendiamo una qualsiasi funzione cognitiva (es. la Memoria di Lavoro), essa ha una complessità e una serie di sfaccettature che raramente sono indagate nel dettaglio in una valutazione neuropsicologica.
...
## Focus: Focus: utilizzare i punteggi ai test per comunicare{#focus-punteggi-comunicare}
Uno dei potenziali utilizzi del test è quello di usare come una sorta di "linguaggio" per facilitare la comunicazione tra clinici. Invece di utilizzare delle lunghe o vaghe spiegazioni che descrivano la/il paziente e le sue caratteristiche il punteggio al test ha la potenzialità comunicare molte informazioni in maniera immediata. Sebbene questo sia assolutamente ragionevole, di seguito saranno forniti due esempi che illustrano alcuni limiti di questo approccio, anche alla luce di tutto quanto è stato trattato in questo capitolo. 
