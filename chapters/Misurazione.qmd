...
:::{.callout-tip}
## Il mio test è adeguato?
Per alcuni tipi di validità e affidabilità la letteratura riporta valori soglia indicativi, ma non esiste un criterio universalmente condiviso per stabilire quando un test sia "adeguato". Per questo motivo, più che chiedersi se un test sia sufficiente o insufficiente, conviene ragionare in termini comparativi: un test ha più o meno evidenze di validità e affidabilità, ed è migliore o peggiore rispetto alle alternative disponibili per lo stesso scopo. I valori soglia riportati in questo capitolo vanno quindi letti come orientamenti pratici, non come standard assoluti, in linea con l'idea di un continuum Peggio ↔ Meglio, piuttosto che di una dicotomia netta.
:::

Validità e affidabilità sono le due proprietà fondamentali a cui fare riferimento per stabilire se un test è un buon test, e si collegano direttamente al principio cardine della CTT ($X = T + E$; cfr. paragrafo _Il principio cardine: X = T + E_). La validità risponde alla domanda _cosa stiamo misurando?_, cioè se il punteggio vero $T$ rifletta effettivamente il costrutto che il test intende misurare. L'affidabilità risponde invece alla domanda _quanto precisamente lo misuriamo?_, cioè quanto è piccola la componente d'errore $E$ rispetto al punteggio osservato $X$. Nei prossimi paragrafi tratteremo prima la validità e poi, in una sezione dedicata, l'affidabilità.
...
:::{.callout-note}
## Definizione
_"La validità è il grado in cui un test misura effettivamente il costrutto che intende misurare."_ `(\cite{nunnally1994psychometric})`{=latex}`(Nunnally & Bernstein, 1994)`{=html}
:::

Un aspetto spesso trascurato è che la validità non è una proprietà generale e assoluta di un test, ma è sempre riferita a un **utilizzo specifico**: non ha senso dire semplicemente che "un test è valido", mentre ha senso dire che un test è valido per un certo scopo, in una certa popolazione, in un certo contesto. Uno stesso test di memoria può avere buone evidenze di validità per identificare un decadimento cognitivo in soggetti anziani, e nessuna evidenza per, ad esempio, predire il rendimento scolastico in età evolutiva.

Come per l'affidabilità, anche la validità è più utile pensarla come un **continuum**, piuttosto che come una proprietà binaria (valido/non valido): un test accumula, nel tempo e attraverso diversi studi, un insieme di evidenze a favore (o contro) un determinato utilizzo, e la nostra fiducia nel test dovrebbe aggiornarsi di conseguenza.
...
:::{.callout-note}
## Definizione
La validità di facciata è il grado in cui un test **sembra** misurare ciò che intende misurare, sulla base di un giudizio superficiale e non di dati empirici.
:::
...
La validità di facciata è la forma di validità meno rigorosa: viene valutata solo qualitativamente, senza alcuna analisi statistica. Talvolta è l'unica evidenza disponibile per un test, ma la sua plausibilità superficiale non è sufficiente per concludere che lo strumento misuri davvero il costrutto di interesse[^qmp].

[^qmp]: Un esempio didattico spesso usato per illustrare questo punto è quello di un ipotetico "questionario di qualità psicologica" (QMP), che appare del tutto plausibile a prima vista, ma che una volta verificato empiricamente si rivela inaffidabile e privo di reali evidenze di validità.
...
I valori soglia riportati in questo paragrafo sono **indicativi e non universali**: la letteratura non è unanime, e non è raro che test pubblicati e ampiamente utilizzati circolino con valori di affidabilità ben al di sotto di quelli raccomandati `(\cite{slick2006psychometrics})`{=latex}`(Slick, cap. 1 in Spreen & Strauss, 2006)`{=html}. Una classificazione generale della grandezza dei coefficienti di affidabilità, che vale come riferimento orientativo per tutti i tipi di affidabilità trattati di seguito, è la seguente (Slick, cap. 1):
...
Per le decisioni cliniche, Sattler `(\cite{sattler2001assessment})`{=latex}`(Sattler, 2001)`{=html} propone soglie più stringenti: per i test usati nella valutazione del singolo individuo si raccomanda un'affidabilità pari o superiore a .80; per i test usati per decisioni particolarmente importanti (per esempio la stima del QI o una diagnosi), .90 è considerato il minimo assoluto, e .95 il valore ottimale.
...
1. **Dipendenza dall'intervallo**: uno stesso test non ha un'unica affidabilità test-retest, ma infinite, perché il valore cambia a seconda dell'intervallo scelto tra le due somministrazioni, ed è tipicamente più alto con intervalli brevi (Anastasi & Urbina, 1997, citati in Slick, cap. 1).
2. **Effetto pratica**: se il test misura una prestazione, la seconda somministrazione potrebbe beneficiare dell'esperienza acquisita nella prima. Un simile effetto sistematico non viene catturato dal coefficiente di correlazione, che è sensibile a quanto i punteggi si muovono insieme, ma non a un eventuale cambiamento sistematico del livello medio dei punteggi (Slick, cap. 1).
...
### Come faccio a sapere se il mio test è adeguato?

:::{.callout-tip}
## Come documentarsi
- **Il primo posto in cui cercare** è il manuale del test, oppure l'articolo originale con cui lo strumento è stato presentato: è lì che si trovano le evidenze di validità e affidabilità raccolte dagli autori al momento della pubblicazione.
- **Le evidenze non si esauriscono con la pubblicazione**: un test può accumulare nuove prove di validità e affidabilità anche molti anni dopo la sua uscita. Per esempio, la Frontal Assessment Battery (FAB), pubblicata da Appollonio et al. nel 2005, ha visto pubblicare nuove evidenze psicometriche in Aiello et al. (2022), quasi vent'anni dopo. Vale quindi la pena cercare, oltre all'articolo originale, anche gli **studi successivi** che lo citano (per esempio tramite Google Scholar o PubMed), per verificare se sono emerse nuove evidenze in popolazioni o contesti diversi.
- **Il fatto che un test sia pubblicato, anche da autori esperti, non garantisce di per sé nulla**: uno strumento pubblicato può comunque avere una bassa affidabilità, o nessuna evidenza di validità per lo specifico utilizzo che se ne vuole fare. Valutare criticamente queste evidenze fa parte del lavoro di chi utilizza il test in ambito clinico.
:::

Come già sottolineato all'inizio di questa sezione, le evidenze di validità e affidabilità si accumulano nel tempo, e vanno lette come un continuum piuttosto che come un giudizio definitivo: usare oggi un test con prove ancora limitate non equivale automaticamente a usare un test "non valido", ma significa muoversi verso il polo peggiore del continuum. È anche per questo motivo che l'aggiornamento professionale continuo è così rilevante in questo ambito: nuove prove pubblicate nel tempo possono spostare la valutazione complessiva di uno strumento, in meglio o in peggio.
