...
Finora abbiamo detto che nella valutazione neuropsicologica sono utilizzati test per ottenere informazioni su sul funzionamento cognitivo del paziente o in generale informazioni importanti a livello clinico. Ma quali sono nello specifico gli oggetti di interesse?

Vale la pena fermarci un secondo a riflettere su questo aspetto perchè non esiste un riferimento unanimamente accettato su quali sono gli aspetti che andrebbero valutati in una valutazione neuropsicologica. I riferimenti più importanti sono le Linee Guida nazionali e internazionali specifici per patologia di interesse.
...
Alcuni manuali classici sulla valutazione neuropsicologica, sia disponibili in Italiano, sia in inglese (Citazioni Lezak, citazione Manuale di Neuropsicologia Vallar & Papagno), possono avere specifici capitoli dedicati a patologie o domini da valutare (anche in questo caso con possibili suggerimenti sugli strumenti da usare). 
Queste indicazioni sono però sempre orientative e i test effettivamente disponibili sono sempre molto di più di quelli che sono citati in un manuale generico sulla'attività clinica. Questo è anche perchè un manuale è inevitabilmente una fotografia di un momento nel tempo e può non essere aggiornato alle più recenti linee guida o sviluppi.
...
Nel contesto di questa grande eterogeneità, da un punto di vista più metodologico è però utile fare delle considerazioni generali su come possono essere raggruppati gli oggetti di interesse degli strumenti neuropsicologici (si veda anche Mondini Cappelletti e Arcara 2022).

- _funzioni cognitive o costrutti specifici_: il primo è più classico oggetto di interesse per un test neuropsicologico è un costrutto psicologico e nello specifico, una funzione cognitiva - come potrebbe essere la memoria a breve termine, o l'attenzione selettiva. Questi test fanno parte spesso della valutazione chiamata di _secondo livello_ cioè quella che approfondisce una valutazione iniziale, più ampia, ma approssimativa, fatta con test detti di _screening_. La valutazione neuropsicologica classica è però considerata come un insieme di test approfonditi che vanno ad esaminare vari aspetti del funzionamento cognitivo, proprio tramite una serie di test volti ad indagare funzioni cognitive specifiche. Anche se ogni test non misura mai esattamente un costrutto specifico (citazione Sivan & Benton, Lezak), è utile considerare questi test come il più alto livello di granularità che può essere raggiunto. 
Si noti come gran parte dei test che fanno parte di questo gruppo sono utilizzati perché associati ad un'utilità clinica o perchè sono importanti storicamente (es. il Token Test, De Renzi e Vignolo). Ad un'analisi più approfondita non è però chiaro quale debba essere il livello di dettaglio da raggiungere un test per una valutazione neuropsicologica che viene usato per indagare l'integrità di una funziona cognitiva. Se infatti prendiamo una qualsiasi funzione cognitiva (es. la Memoria di Lavoro), essa ha una complessità e una serie di sfaccettature che raramente sono indagate nel dettaglio in una valutazione neuropsicologica. 
...
1. _costrutti più generici_: l'oggetto di interesse di test neuropsicologici può essere dei costrutti più generici e utili. Il classico esempio è il "funzionamento cognitivo globale", il costrutto a cui fanno riferimento molti test di screening (es. Mondini et al. 2022 GEMS), è un costrutto generico utilizzato per la sua utilità clinica. Un altro esempio è il funzionamento delle funzioni esecutive come misurato da una batteria coma la FAB (citazione FAB e FAB-it). In questo caso il costrutto è un concetto psicologico che è riconoscciuto come scomponibile in aspetti più sfaccettati, ma per cui una semplificazione è utile a livello clinico. Questi test sono spesso considerati come di _primo livello_ perché forniscono una valutazione iniziale, rapida nel dire che c'è qualcosa che merita di essere approfondita tramite test più specifici (che fanno parte dei test su _costrutti specifici_). 
Con questa tipologia di costrutti non c'è quasi mai un corrispettivo a livello neurale. 

2. _costrutti legati ad abilità funzionali_: alcuni costrutti possono essere complessi, ma utili ai fini clinici e forensi. A differenza dei costrutti elencati nel punto precedente questi non sono concetti generici da un punto di vista di modelli cognitivi, ma rappresentano specifici concetti della vita quotidiana, che è verosimile siano legati al funzionamento complesso di diverse funzioni cognitive di base. Sono un esempio la capacità di fornire un testamento (citazione Bolognini), la capacità di gestire la proprie finanze (citazione Arcara NADL-F e citazione Marson), la capacità di guidare.

3. _altri costrutti di potenziale rilevanza clinica_: strettamente legato al paragrafo [Altri strumenti utili per la valutazione neuropsicologica](#altri-strumenti-utili), esistono anche altri costrutti utili che possono riguardare altri ambiti (come la riserva cognitiva, gli stili di vita, lo stato socio economico). Di solito questi costrutti rappresentano delle informazioni che sono utili per la 

4. _probabilità di appartenere ad un gruppo (diagnostico)_: alcuni test molto diffusi non misurano, strettamente parlando, nessun costrutto, ma sono utilizzati come strumento per valutare la probabilità di appartenere ad un determinato gruppo diagnostico. Questi test sono all'apparenza simili ad ogni altro test neuropsicologico (gli item che li compongono sono prove cognitive), ma sono sviluppati non per misurare specifici costrutti (come la memoria), ma per essere capaci di distinguere tra gruppi, generalmente tra condizioni cliniche specifiche (Es. Malattia di Alzheimer o Mild Cognitive Impairment), da condizioni di assenza di patologie note (i.e. essere "sani"). L'utilizzo di questo tipologia di test è trattata ampiamente nel capitolo 4 di questo libro e nella sezione corrispondente degli _Apprfondimenti tecnici_.

Questa possibile divisione non riesce però a classificare in maniera completa e univoca tutti i test neuropsicologici. In particolare alcuni test storicamente rilevanti e tuttora molto diffusi potrebbero ricadere facilmente come parte del primo o del secondo gruppo. Sono esempi i _test di Fluenza (fonemiche o semantiche)_ (Citazione recente qui), il _Test dell'Orologio_ (Citazione qui), il _Token Test_ (Citazioni De renzi). Sono tutti test con una lunga storia (Citazione Spinnler e Tognoni qui), ma che possono essere visti secondo due maniera: o sono test in cui la performance è legata ad una serie di funzioni cognitive (es. per completare il test dell'Orologio serve recupero dalla memoria, pianificazione, abilità prassiche, etc.), oppure è possibile postulare un costrutto più generico, clinico, che le racchiude tutte e che è misurato da quel test.
Notare questo non è solo un esercizio di stile, ma è fondamentale per andare a valutare poi se il test funziona in maniera appropriata oppure no (metti riferimento a validità di costrutto.)
